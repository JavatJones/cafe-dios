...
const ImageGestorUploader = ({ id, name, slug, imageUrl }: getThings) => {
    ...
    const handleUpload = async (e: ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (data) {
                const path = data.path
                const response = await supabase.from("products-categories-n2")
    ...
    return (
        <Dialog>
            ...
            <DialogContent>
                <DialogHeader>
                    <DialogTitle className='truncate'>Gestionar imagen de {name}</DialogTitle>
                </DialogHeader>
